...
export const Pagination: React.FC<PaginationProps> = ({
  ...
  const { page, limit } = pagination // page is 0-based
  ...
  const getPageNumbers = () => {
    ...
    for (
      ...
      i <= Math.min(totalPages - 1, page + 1 + delta);
      i++
    ) {
      range.push(i)
    }
    ...
    <div className="flex items-center justify-between p-6 bg-gradient-to-r from-gray-800/50 to-gray-850/50 rounded-xl border border-gray-700/30">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="text-sm text-gray-400">
          Page <span className="font-semibold text-gray-100">{page + 1}</span> of{' '}
          ...
          {pageNumbers.map((pageNum, index) => (
            <React.Fragment key={index}>
              {pageNum === '...' ? (
                ...
                <BaseButton
                  variant={pageNum === page + 1 ? 'primary' : 'ghost'}
                  size="sm"
